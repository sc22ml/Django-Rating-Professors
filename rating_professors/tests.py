...
class ProfessorRatingTests(APITestCase):
    def setUp(self):
        ...
        self.module_instance = ModuleInstance.objects.create(
            module=self.module,
            year=2023,  # Add the required year field
            semester=1   # Add the required semester field
        )
    ...
    def test_view_professor_ratings(self):
        url = reverse('professor-ratings')  # Ensure this matches the name in urls.py
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data[0]['average_rating'], 5)  # Check if the average rating is correct
...
class ProfessorModuleRatingTests(APITestCase):
    ...
    def test_view_professor_module_rating(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['average_rating'], 4)  # Check if the average rating is correct
...
class RateProfessorTests(APITestCase):
    ...
    def test_rate_professor(self):
        ...
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Rating.objects.count(), 1)  # Check if the rating was created